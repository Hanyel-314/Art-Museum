...
function createPaintingWhiteLightTransition(artworkData) {
    ...
    setTimeout(() => {
        ...
        // Pure black background - no fog, no depth effects
        scene.background = new THREE.Color(0x000000);
        scene.fog = null; // Remove all fog effects
    ...
    // Front canvas (painted surface) - using BasicMaterial for pure color
    const frontGeo = new THREE.PlaneGeometry(size, size);
    const frontMat = new THREE.MeshBasicMaterial({
        color: artworkData.color // Pure color, unaffected by lighting
    });
    const front = new THREE.Mesh(frontGeo, frontMat);
    front.position.z = 0;
    detailArtwork.add(front);

    // Simple frame - using BasicMaterial for pure gold color
    const frameMat = new THREE.MeshBasicMaterial({
        color: 0xD4AF37 // Pure gold color
    });
    ...
    selectedPainting = artworkData;
        ...
        // Minimal ambient light - just enough to see the painting
        detailKeyLight = new THREE.AmbientLight(0xFFFFFF, 0.8);
